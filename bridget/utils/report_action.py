...
from bridget.utils.mod import warn
from bridget.utils.utils import send_error
...
class ReportActionReason(ui.View):
    ...
    async def modaction_callback(self, interaction: discord.Interaction, reason: str):
        if self.mod_action == ModAction.WARN:
            ...
            await interaction.guild.ban(self.target_member, reason=reason)
...
class WarnView(ui.View):
    ...
    async def on_timeout(self) -> None:
        await send_error(self.ctx, "Timed out.")
