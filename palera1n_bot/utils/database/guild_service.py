...
def add_raid_phrase(self, phrase: str) -> bool:
    ...
    if(len(existing) > 0):
        return False
    Guild.objects(_id=guild_id).update_one(push__raid_phrases=FilterWord(word=phrase, bypass=5, notify=True))
    return True

def remove_raid_phrase(self, phrase: str):
    Guild.objects(_id=guild_id).update_one(pull__raid_phrases__word=FilterWord(word=phrase).word)
