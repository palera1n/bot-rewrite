...
class ChatGPT(Cog, commands.GroupCog, group_name="chatgpt"):
    ...
    async def on_message(self, message: discord.Message) -> None:
        if (message.channel.id != guild_service.get_guild().channel_chatgpt
                or message.author.bot or message.content.startswith(
                    ("--", "–", "—", "<@"))):
            return
        ...
        async with message.channel.typing():
            try:
                try:
                    res = await self.context[message.author.id
                ...
                if len(res) <= 2000:
                    ...
                    await message.reply(
                        ...
                        file=discord.File(io.BytesIO(wrapped.encode()),
