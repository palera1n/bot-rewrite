...
class NativeActionsListeners(Cog):
    ...
    async def on_member_remove(self, member: discord.Member):
        guild = member.guild
        audit_logs = [
            audit async for audit in guild.audit_logs(
                limit=1,
        ...
        if audit_logs and audit_logs[0].target == member:
            ...
            await channel.send(embed=await add_kick_case(
                member, audit_logs[0].user, "No reason." if audit_logs[0].
                reason is None else audit_logs[0].reason,
                guild_service.get_guild(), self.bot))
    ...
    async def on_member_ban(self, guild: discord.Guild,
        ...
        if audit_logs and audit_logs[0].target == member:
            channel = guild.get_channel(
                guild_service.get_guild().channel_public)
            await channel.send(embed=await add_ban_case(
                member, audit_logs[0].user, "No reason." if audit_logs[0].
                reason is None else audit_logs[0].reason,
                guild_service.get_guild(), self.bot))
    ...
    async def on_member_unban(self, guild: discord.Guild,
        ...
        audit_logs = [
            audit async for audit in guild.audit_logs(
                ...
                after=member.created_at)
        ]
        if audit_logs and audit_logs[0].target == member:
            channel = guild.get_channel(
                guild_service.get_guild().channel_public)
            await channel.send(embed=await add_unban_case(
                member, audit_logs[0].user, "No reason." if audit_logs[0].
                reason is None else audit_logs[0].reason,
                guild_service.get_guild(), self.bot))
    ...
    async def on_member_update(self, before: discord.Member,
        ...
        if not before.is_timed_out() and after.is_timed_out():
            ...
            audit_logs = [
                audit async for audit in after.guild.audit_logs(
                    limit=1,
                    action=discord.AuditLogAction.member_update,
                    after=after.joined_at)
            ]
            if audit_logs and audit_logs[0].target == after:
                await channel.send(embed=await add_mute_case(
                    after, audit_logs[0].user, "No reason." if audit_logs[0].
                    reason is None else audit_logs[0].reason,
                    guild_service.get_guild(), self.bot))
    ...
    async def on_automod_action(self, ctx: discord.AutoModAction):
        await ctx.channel.send(
            f"{ctx.member.name} sent `{ctx.content}` and triggered automod!")
